...
export default function Home() {
  ...
  const [messages, setMessages] = useState<Array<{ id: string; role: 'user' | 'agent'
        ...
        <aside className="flex flex-col overflow-hidden" style={{ background: 'var(--sidebar)', borderRight: '1px solid var(--sidebar-b)' }}>
          ...
          <div className="flex-1 overflow-y-auto px-2 pb-2">
            ...
            {customTeams.map(team => {
              ...
              return (
                <div key={team.id} onClick={() => { setPage('office'); setActiveAgentId(team.id as AgentId) }}
                  className="px-3 py-2 flex items-center gap-2.5 cursor-pointer rounded-xl mb-0.5 text-[12px] transition-all"
                  style={{ background: isActive ? 'var(--sidebar-b)' : 'transparent', borderLeft: `2px solid ${isActive ? color : 'transparent'}`, color: isActive ? color : '#ffffff' }}>
                  <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${isActive ? 'animate-pulse' : ''}`} style={{ background: isActive ? color : 'var(--blush-b)' }} />
        ...
        <div className="flex flex-col min-w-0 overflow-hidden" style={{ background: 'var(--bg2)' }}>
          <div className="px-5 py-3 flex items-center gap-3 flex-shrink-0" style={{ background: 'var(--card)', borderBottom: '1px solid var(--border)' }}>
            ...
            {activeAgentId && page === 'office' && (
              <span className="text-[10px] px-2 py-0.5 rounded-full animate-pulse ml-1" style={{ background: 'var(--blush-l)', color: 'var(--blush)', border: '1px solid var(--blush-b)' }}>⚡ 작업 중..</span>
            )}
          </div>
          {page === 'office' && (
        ...
        <div className="flex flex-col overflow-hidden" style={{ background: 'var(--blush-l)', borderLeft: '1px solid var(--blush-b)', height: '100%' }}>
              ...
              <div key={msg.id}>
                {msg.role === 'user' ? (
                  <div className="flex flex-col items-end gap-1">
                    ...
                    <div className="text-[13px] px-3.5 py-2.5 rounded-2xl rounded-tr-sm max-w-[90%] leading-relaxed" style={{ background: 'var(--blush)', color: '#fff' }}>{msg.text}</div>
                  </div>
                ) : (
          ...
          <div className="p-3 flex gap-2 flex-shrink-0" style={{ background: '#fff', borderTop: '1px solid var(--blush-b)' }}>
            <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && !e.shiftKey && sendMessage()}
              disabled={loading} placeholder={loading ? '⏳ 작업 중...' : '업무를 지시해보세요!'}
