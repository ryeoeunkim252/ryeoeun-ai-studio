...
const GITHUB_TOOLS: Anthropic.Tool[] = [
  { name: 'github_list_repos', description: '내 GitHub 저장소 목록을 가져옵니다.', input_schema: { type: 'object', properties: {}, required: [] } },
  { name: 'github_list_issues', description: 'GitHub 저장소의 이슈 목록을 가져옵니다.', input_schema: { type: 'object', properties: { repo: { type: 'string', description: '저장소 이름' } }, required: ['repo'] } },
  { name: 'github_create_issue', description: 'GitHub 저장소에 새 이슈를 생성합니다.', input_schema: { type: 'object', properties: { repo: { type: 'string', description: '저장소 이름' }, title: { type: 'string', description: '이슈 제목' }, body: { type: 'string', description: '이슈 내용' } }, required: ['repo', 'title', 'body'] } },
  { name: 'github_list_commits', description: 'GitHub 저장소의 최근 커밋 목록을 가져옵니다.', input_schema: { type: 'object', properties: { repo: { type: 'string', description: '저장소 이름' } }, required: ['repo'] } },
]
...
async function callGitHubAPI(toolName: string, input: Record<string, string>): Promise<string> {
  ...
  try {
    ...
    if (toolName === 'github_create_issue') {
      ...
      const data = await res.json()
      if (data.message) return JSON.stringify({ error: data.message })
      return JSON.stringify({ success: true, issue_number: data.number, url: data.html_url, message: `✅ 이슈 #${data.number} "${input.title}"이 생성됐어요!` })
    }
...
const GDRIVE_TOOLS: Anthropic.Tool[] = [
  { name: 'gdrive_list_files', description: 'Google Drive의 파일 목록을 가져옵니다.', input_schema: { type: 'object', properties: { query: { type: 'string', description: '검색어 (선택사항)' } }, required: [] } },
  { name: 'gdrive_create_file', description: 'Google Drive에 새 텍스트 파일을 생성합니다.', input_schema: { type: 'object', properties: { name: { type: 'string', description: '파일 이름' }, content: { type: 'string', description: '파일 내용' } }, required: ['name', 'content'] } },
...
async function callGDriveAPI(toolName: string, input: Record<string, string>): Promise<string> {
  try {
    ...
    if (toolName === 'gdrive_create_file') {
      ...
      const data = await res.json()
      return JSON.stringify({ success: true, file_id: data.id, message: `✅ Google Drive에 "${input.name}" 파일을 생성했어요!` })
    }
    return JSON.stringify({ error: '알 수 없는 도구' })
  } catch (err) {
    return JSON.stringify({ error: String(err) })
...
async function callGCalAPI(toolName: string, input: Record<string, string>): Promise<string> {
  try {
    ...
    if (toolName === 'gcal_create_event') {
      ...
      const data = await res.json()
      return JSON.stringify({ success: true, event_id: data.id, link: data.htmlLink, message: `✅ "${input.title}" 일정이 Google Calendar에 추가됐어요!` })
    }
    return JSON.stringify({ error: '알 수 없는 도구' })
  } catch (err) {
    return JSON.stringify({ error: String(err) })
              ...
              if (res.stop_reason === 'tool_use') {
                ...
                for (const block of res.content) {
                  ...
                  if (parsedResult.message) {
                    send({ type: 'text', text: parsedResult.message + '\n', step: i + 1 })
                    fullText += parsedResult.message + '\n'
                  }
