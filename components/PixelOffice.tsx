...
const CHARS: Record<string, {
  ...
  router:  {gender:'M',hair:'#909098',hairStyle:'side',  skin:'#f4c890',top:'#1e3050',bottom:'#121828',shoe:'#0a0a14',acc:'tie',  accent:'#d08888',label:'총괄'},
  web:     {gender:'F',hair:'#1a9aaa',hairStyle:'bob',   skin:'#f4c080',top:'#f8d840',bottom:'#4858a8',shoe:'#283068',acc:'none', accent:'#f8d840',label:'웹팀'},
  content: {gender:'F',hair:'#3a1808',hairStyle:'long',  skin:'#f8d090',top:'#e85888',bottom:'#281828',shoe:'#140a0a',acc:'ear',  accent:'#e85888',label:'콘텐츠'},
  edu:     {gender:'F',hair:'#141414',hairStyle:'pony',  skin:'#f4c080',top:'#4a8040',bottom:'#283028',shoe:'#141414',acc:'none', accent:'#60a050',label:'교육팀'},
  research:{gender:'M',hair:'#281808',hairStyle:'messy', skin:'#f4c080',top:'#e8e8e8',bottom:'#284868',shoe:'#182038',acc:'glass',accent:'#4890d8',label:'연구팀'},
  ops:     {gender:'M',hair:'#0a0a0a',hairStyle:'short', skin:'#c89068',top:'#284828',bottom:'#181e18',shoe:'#080808',acc:'head', accent:'#4a9a4a',label:'운영팀'},
}

const CUSTOM_STYLES = [
  {gender:'F' as const,hair:'#e84080',hairStyle:'bob',  skin:'#f4c080',top:'#40a0e8',bottom:'#284858',shoe:'#183048',acc:'none', accent:'#40a0e8'},
  {gender:'M' as const,hair:'#604020',hairStyle:'short',skin:'#c89060',top:'#a04820',bottom:'#281808',shoe:'#180808',acc:'glass',accent:'#e06030'},
  {gender:'F' as const,hair:'#8040c0',hairStyle:'long', skin:'#f8d090',top:'#60c080',bottom:'#184028',shoe:'#0a1808',acc:'ear',  accent:'#60c080'},
  {gender:'M' as const,hair:'#404040',hairStyle:'messy',skin:'#f0c880',top:'#506090',bottom:'#283048',shoe:'#101828',acc:'head', accent:'#7090c0'},
  {gender:'F' as const,hair:'#d04020',hairStyle:'pony', skin:'#f4c890',top:'#d0a020',bottom:'#482808',shoe:'#281008',acc:'none', accent:'#e0b030'},
  {gender:'M' as const,hair:'#206040',hairStyle:'side', skin:'#c8a070',top:'#206060',bottom:'#182828',shoe:'#081818',acc:'tie',  accent:'#30a080'},
]

const CUSTOM_POSITIONS = [
  {x:284,y:240},{x:284,y:318},{x:46,y:155},{x:165,y:155},{x:284,y:155},{x:46,y:80},
]

const C = {
  bg:'#0d0d1a', fl1:'#c0c0b8', fl2:'#b0b0a8', flg:'#d0d0c8',
  ...
  ct:'#c89850', ctD:'#a07030', fr:'#d4a030', frD:'#a07820',
  cup:'#e88060', cupH:'#f0a080', book1:'#e84040', book2:'#4080e0',
}
...
export default function PixelOffice({ activeAgentId }: Props) {
  ...
  const customsRef = useRef<CustomTeam[]>([])
  ...
  useEffect(()=>{
    ...
    customsRef.current = loadData<CustomTeam[]>('nk_custom_teams', [])
  },[])
  ...
  useEffect(()=>{
    ...
    const drawMonitor=(x:number,y:number,t:number)=>{
      ...
      r(x,y,32,24,C.mon);r(x+2,y+2,28,20,'#14182a');r(x+3,y+3,26,16,C.monS)
      ctx.globalAlpha=p*0.85
      r(x+4,y+4,12,3,C.monG);r(x+4,y+9,22,2,'#88c0e0');r(x+4,y+13,16,2,C.monG)
      ctx.globalAlpha=1
      r(x+12,y+24,8,6,C.mon);r(x+8,y+30,16,3,'#14182a')
    }

    const drawChair=(x:number,y:number,col:string)=>{
      ctx.fillStyle='rgba(0,0,0,0.15)';ctx.fillRect(x+3,y+34,22,5)
      r(x,y,28,6,col);r(x+2,y+2,24,4,col+'cc')
      r(x,y+6,4,20,col);r(x+24,y+6,4,20,col)
      r(x,y+6,28,20,col);r(x+2,y+8,24,5,col+'cc')
    ...
    const drawPlant=(x:number,y:number,big=false)=>{
      ...
      r(px2+2,py2+2,pw-4,4,'rgba(255,200,160,0.2)');r(px2+2,py2+3,pw-4,5,'#2a1808')
      r(px2+Math.round(8*s),y+Math.round(10*s),4,Math.round(20*s),'#286020')
      r(x,y,Math.round(20*s),Math.round(16*s),C.plt2);r(x,y,Math.round(17*s),Math.round(10*s),C.plt1)
    ...
    const drawSofa=(x:number,y:number,w=72)=>{
      ...
      r(x,y,w,10,C.sfA);r(x+3,y+2,w-6,6,C.sfD);r(x,y+10,w,34,C.sf)
      r(x+3,y+12,w-6,8,C.sfL);r(x,y+44,w,10,C.sfA)
      r(x,y+10,10,34,C.sfD);r(x+w-10,y+10,10,34,C.sfD)
      r(x+1,y+11,4,30,'rgba(255,255,255,0.1)')
    }

    const drawCoffeeTable=(x:number,y:number,w=62,h=40)=>{
      ctx.fillStyle='rgba(0,0,0,0.15)';ctx.fillRect(x+5,y+h+5,w-6,7)
      r(x,y,w,h,C.ct);r(x+3,y+3,w-6,6,'rgba(255,220,120,0.3)')
      r(x,y,3,h,C.ctD);r(x+w-3,y,3,h,C.ctD);r(x,y+h,w,8,C.ctD)
    ...
    const drawFrame=(x:number,y:number,ci:number)=>{
      ...
      r(x,y,32,24,C.fr);r(x+3,y+3,26,18,C.frD)
      r(x+4,y+4,24,16,arts[ci%arts.length]);r(x+5,y+5,10,6,'rgba(255,255,255,0.2)')
    }

    const drawDeskItems=(x:number,y:number,type:number)=>{
      if(type===0){r(x,y+2,10,10,C.cup);r(x+1,y+2,8,3,C.cupH);r(x+2,y+3,6,3,'#3a1a08')}
      else if(type===1){r(x,y,5,12,C.book1);r(x+6,y+2,5,10,C.book2);r(x,y,1,12,'rgba(255,255,255,0.25)')}
      else{r(x+2,y+6,8,8,'#d06030');r(x+5,y,5,8,'#40a040');r(x+3,y+1,3,6,'#50c050')}
    }

    type CharData = {gender:'M'|'F';hair:string;skin:string;top:string;bottom:string;shoe:string;hairStyle:string;acc:string;accent:string;label:string}

    const drawChar=(px:number,py:number,ch:CharData,walking:boolean,frame:number,highlight:boolean,name:string,showBubble:boolean,bubbleText:string,sitting=false)=>{
      const iF=ch.gender==='F'
      if(highlight){ctx.fillStyle=ch.accent+'55';ctx.beginPath();ctx.arc(px+14,py+22,20,0,Math.PI*2);ctx.fill()}
      ...
      else if(ch.hairStyle==='pony'){r(px+3,py,20,8,ch.hair);r(px+19,py+7,5,10,ch.hair);r(px+2,py+7,5,8,ch.hair);r(px+22,py+12,9,22,ch.hair)}
    ...
    const walkers:[number,number,number,number,string,number,number][]=[
      [80,250,220,290,'research',0,0.85],
      [320,270,90,240,'ops',0,0.95],
    ]
    const BUBBLE:Record<string,string>={
      router:'업무 배분 중',web:'디자인 작업중',content:'콘텐츠 제작',
      edu:'교육 기획중',research:'데이터 분석',ops:'시스템 관리',
    }
    const CHAIR_COLORS=['#7868a8','#688898','#988870','#809870','#886888','#708868','#886870','#687088']
    const SEATS=[
      {x:46,y:240,charId:'router',dw:68},{x:165,y:228,charId:'web',dw:68},
      {x:46,y:318,charId:'content',dw:68},{x:165,y:318,charId:'edu',dw:68},
    ]

    let animId:number
    const loop=()=>{
      tickRef.current+=0.04
      const t=tickRef.current,active=activeRef.current
      const s=settingsRef.current,customs=customsRef.current

      ctx.fillStyle=C.bg;ctx.fillRect(0,0,cv.width,cv.height)

      const R1={x:4,y:4,w:366,h:388}
      drawRoom(R1.x,R1.y,R1.w,R1.h)
      drawShelf(R1.x+14,R1.y+8,162);drawShelf(R1.x+188,R1.y+8,162)
      drawClock(R1.x+R1.w/2-1,R1.y+28,t)
      drawPlant(R1.x+12,R1.y+60,true);drawPlant(R1.x+R1.w-48,R1.y+60,false)
      ...
      SEATS.forEach(({x,y,charId,dw},i)=>{
        ...
        drawChar(x+dw/2-14,y-5+bob,CHARS[charId],false,0,isActive,name,isActive,BUBBLE[charId]||'작업중',true)
      })

      // ✅ 커스텀 팀 책상 + 캐릭터 자동 생성
      customs.slice(0,6).forEach((team,i)=>{
        const pos=CUSTOM_POSITIONS[i]; if(!pos) return
        const dw=68,{x,y}=pos
        const isActive=active===(team.id as AgentId)
        const name=s.agentNames?.[team.id]||team.name
        const style={...CUSTOM_STYLES[i%CUSTOM_STYLES.length],label:team.name}
        drawChair(x+dw+5,y-2,CHAIR_COLORS[(i+4)%CHAIR_COLORS.length])
        drawDesk(x,y,dw,38)
        drawMonitor(x+dw/2-4,y-34,t*(1.2+i*0.17))
        drawDeskItems(x+4,y+4,(i+1)%3)
        const bob=Math.sin(t*1.7+(i+4)*0.9)*0.8
        drawChar(x+dw/2-14,y-5+bob,style,false,0,isActive,name,isActive,'작업중',true)
      })

      // 걸어다니는 팀
      walkers.forEach((w,i)=>{
        const[,,tx,ty,charId]=w
        const dx=tx-w[0],dy=ty-w[1],dist=Math.sqrt(dx*dx+dy*dy)
        if(dist>2){w[0]+=dx/dist*(w[6]);w[1]+=dy/dist*(w[6])*0.65;w[5]=Math.floor(t*5)%4}
        else{w[2]=R1.x+20+Math.random()*300;w[3]=R1.y+70+Math.random()*270;w[5]=0}
        const isActive=active===charId
        const name=s.agentNames?.[charId]||CHARS[charId].label
        drawChar(Math.round(w[0]),Math.round(w[1]),CHARS[charId],true,w[5],isActive,name,isActive,BUBBLE[charId])
      })

      const R2={x:376,y:4,w:238,h:388}
      drawRoom(R2.x,R2.y,R2.w,R2.h)
      drawFrame(R2.x+18,R2.y+12,0);drawFrame(R2.x+110,R2.y+12,1);drawFrame(R2.x+186,R2.y+12,2)
      drawPlant(R2.x+12,R2.y+62,true);drawPlant(R2.x+R2.w-46,R2.y+62,false)
      drawSofa(R2.x+14,R2.y+84,84);drawSofa(R2.x+118,R2.y+84,84)
      drawSofa(R2.x+60,R2.y+270,100);drawCoffeeTable(R2.x+76,R2.y+160)
      {
        const isActive=active==='ops',name=s.agentNames?.ops||CHARS.ops.label
        const bob=Math.sin(t*1.4)*0.7
        drawChar(R2.x+68,R2.y+110+bob,CHARS.ops,false,0,isActive,name,isActive,BUBBLE.ops)
      }
