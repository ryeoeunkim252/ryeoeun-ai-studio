...
export default function Dashboard() {
  ...
  const agentIds = ['router', 'web', 'content', 'edu', 'research', 'ops']
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-[13px] mt-1" style={{ color: 'var(--muted)' }}>AI 에이전트 활동 현황</p>
          </div>
          ...
          <div className="rounded-2xl p-5" style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
            ...
            <div className="flex flex-col gap-2">
              {agentIds.map(id => {
                ...
                const name = settings.agentNames?.[id] || id
                const color = AGENT_ACCENT[id] || 'var(--blush)'
                return (
                  <div key={id} className="flex items-center gap-3 p-3 rounded-xl"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[10px] mb-1.5" style={{ color: 'var(--muted)' }}>{AGENT_ROLES[id]}</div>
                      {/* 활동 바 */}
                    ...
                    .map(([id, count]) => {
                      ...
                      return (
                        <div key={id} className="flex items-center gap-2">
                          <span className="text-sm w-5">{AGENT_ICONS[id]}</span>
                ...
                <div className="flex flex-col gap-2">
                  {logs.slice(-4).reverse().map(log => (
                    <div key={log.id} className="flex items-start gap-2 p-2.5 rounded-xl"
                      style={{ background: 'var(--bg2)' }}>
                      <span className="text-sm flex-shrink-0">{AGENT_ICONS[log.agentId] || '🤖'}</span>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1 mb-0.5">
                          <span className="text-[11px] font-medium" style={{ color: 'var(--text)' }}>{log.agentName}</span>
