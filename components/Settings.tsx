...
// ── 새 조직도 반영 (팀 ID는 pipeline 호환 유지) ──────────────
const DEFAULT_AIDS = ['router', 'content', 'research', 'ops', 'web', 'edu']
...
const DEFAULT_ROLES: Record<string, string> = {
  router:   '총괄실장 · PM+COO',
  content:  '콘텐츠팀장 · 기획·디자인·채널',
  research: '전략실장 · 시장·트렌드',
  ops:      '자동화팀장 · 워크플로우·API',
  web:      '수익화팀장 · 수익·제휴',
  edu:      '데이터팀장 · CRM·분석',
}
...
const DEFAULT_DESC: Record<string, string> = {
  ...
  content:  '기획·카피·디자인·채널 운영을 총괄해요',
  research: '시장 분석과 트렌드 조사, 전략 기획을 해요',
  ops:      '워크플로우 자동화와 API 연결을 담당해요',
  web:      '제휴 마케팅과 수익화 구조를 설계해요',
  edu:      '데이터 분석과 고객 CRM을 관리해요',
}
...
const MCPS = [
  { id: 'figma',           icon: '🎨', name: 'Figma',          desc: '디자인 작업, 컴포넌트 생성',          defaultTeams: ['content'] },
  { id: 'notion',          icon: '📝', name: 'Notion',         desc: '문서 관리, 데이터베이스',              defaultTeams: ['research', 'ops'] },
  { id: 'vercel',          icon: '▲',  name: 'Vercel',         desc: '배포, 도메인 관리',                   defaultTeams: ['ops'] },
  { id: 'github',          icon: '🐙', name: 'GitHub',         desc: '코드 관리, PR 리뷰',                  defaultTeams: ['ops'] },
  { id: 'gmail',           icon: '📧', name: 'Gmail',          desc: '이메일 발송 및 관리',                  defaultTeams: ['edu'] },
  ...
  { id: 'jira',            icon: '🎯', name: 'Jira',           desc: '프로젝트 관리, 스프린트 관리',          defaultTeams: ['ops'] },
  { id: 'airtable',        icon: '📊', name: 'Airtable',       desc: '데이터 관리, 스프레드시트 자동화',       defaultTeams: ['edu'] },
  { id: 'zapier',          icon: '🔗', name: 'Zapier',         desc: '앱 연동, 자동화 워크플로우',            defaultTeams: ['ops'] },
  { id: 'hubspot',         icon: '🏢', name: 'HubSpot',        desc: 'CRM, 마케팅 자동화',                  defaultTeams: ['edu', 'web'] },
...
export default function Settings() {
  ...
  const allIds = teamOrder.length > 0
    ? teamOrder
        .filter(id => [...DEFAULT_AIDS, ...customTeams.map(t => t.id)].includes(id))
  ...
  const resetNavOrder = () => { setNavOrder(DEFAULT_NAV_ITEMS.map(n => n.id)) }
  ...
  // ══════════════════════════════════════════════════════════
  // ✅ BUG FIX: async/await로 Supabase 저장 완료 후 reload
  //    (기존: saveData 비동기인데 await 없이 reload → Race Condition)
  // ══════════════════════════════════════════════════════════
  const saveAll = async () => {
    await Promise.all([
      ...
      saveData('nk_nav_order', navOrder),
    ])
    setSaved(true)
      ...
      <div className="max-w-2xl mx-auto flex flex-col gap-5">
        ...
        <div className="rounded-2xl p-5" style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
          <div className="flex items-center justify-between mb-1">
            ...
            <button onClick={resetNavOrder} className="text-[11px] px-3 py-1 rounded-lg"
              style={{ background: 'var(--bg2)', color: 'var(--muted)', border: '1px solid var(--border)' }}>초기화</button>
          </div>
          <p className="text-[12px] mb-4" style={{ color: 'var(--muted)' }}>왼쪽 사이드바 메뉴의 순서를 ↑↓ 버튼으로 바꿀 수 있어요</p>
          <div className="flex flex-col gap-2">
            {sortedNavItems.map((item, idx) => (
              <div key={item.id} className="flex items-center gap-3 px-4 py-3 rounded-xl"
                style={{ background: 'var(--bg2)', border: '1px solid var(--border)' }}>
                <span style={{ width:20,height:20,borderRadius:'50%',flexShrink:0,background:'var(--blush)',color:'#fff',fontSize:10,fontWeight:700,display:'flex',alignItems:'center',justifyContent:'center' }}>{idx+1}</span>
                <span style={{ fontSize: 16 }}>{item.icon}</span>
                <span style={{ flex:1, fontSize:13, fontWeight:500, color:'var(--text)' }}>{item.label}</span>
                <div className="flex gap-1.5">
                  <button onClick={() => moveNavUp(item.id)} disabled={idx===0}
                    style={{ width:28,height:28,borderRadius:7,border:'none',background:idx===0?'var(--border)':'var(--blush)',color:idx===0?'var(--muted)':'#fff',fontSize:12,cursor:idx===0?'not-allowed':'pointer',opacity:idx===0?0.4:1,display:'flex',alignItems:'center',justifyContent:'center' }}>▲</button>
                  <button onClick={() => moveNavDown(item.id)} disabled={idx===sortedNavItems.length-1}
                    style={{ width:28,height:28,borderRadius:7,border:'none',background:idx===sortedNavItems.length-1?'var(--border)':'var(--blush)',color:idx===sortedNavItems.length-1?'var(--muted)':'#fff',fontSize:12,cursor:idx===sortedNavItems.length-1?'not-allowed':'pointer',opacity:idx===sortedNavItems.length-1?0.4:1,display:'flex',alignItems:'center',justifyContent:'center' }}>▼</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="rounded-2xl p-5" style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
          ...
          <p className="text-[12px] mb-2" style={{ color: 'var(--muted)' }}>팀 이름·역할·설명 수정 / ↑↓ 순서 변경 / AI 모델 선택</p>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-1 flex flex-col gap-0.5">
                        <input value={getRole(id)} onChange={e => updateRole(id, e.target.value)}
                          placeholder="역할 (예: 콘텐츠팀장 · 기획·디자인·채널)"
          ...
          {showAddTeam && (
            <div className="rounded-xl p-4 mt-3 flex flex-col gap-2" style={{ background:'var(--bg2)',border:'1px solid var(--border)' }}>
              ...
              <div className="flex flex-wrap gap-1.5">
                {ICON_OPTIONS.map(ic => (
                  <button key={ic} onClick={() => setNewTeamIcon(ic)}
                    className="w-8 h-8 rounded-lg text-[16px] flex items-center justify-center"
              ...
              <input value={newTeamName} onChange={e => setNewTeamName(e.target.value)} placeholder="팀 이름"
                ...
                style={{ background:'var(--card)',border:'1px solid var(--border)',color:'var(--text)' }} />
              <input value={newTeamRole} onChange={e => setNewTeamRole(e.target.value)} placeholder="역할"
                className="px-3 py-2 rounded-xl text-[12px] outline-none"
                style={{ background:'var(--card)',border:'1px solid var(--border)',color:'var(--text)' }} />
              <input value={newTeamDesc} onChange={e => setNewTeamDesc(e.target.value)} placeholder="팀 설명"
                className="px-3 py-2 rounded-xl text-[12px] outline-none"
                style={{ background:'var(--card)',border:'1px solid var(--border)',color:'var(--text)' }} />
              ...
              <div className="flex gap-2 mt-1">
                <button onClick={() => { setShowAddTeam(false); setNewTeamName('') }}
                  className="flex-1 py-2 rounded-xl text-[12px]" style={{ background:'var(--border)',color:'var(--muted)' }}>취소</button>
                <button onClick={addTeam}
                  className="flex-1 py-2 rounded-xl text-[12px]" style={{ background:'var(--blush)',color:'#fff' }}>+ 추가</button>
              </div>
            </div>
          )}
          {!showAddTeam && (
            <button onClick={() => setShowAddTeam(true)}
              className="w-full mt-3 py-2.5 rounded-xl text-[13px] flex items-center justify-center gap-2"
        ...
        <div className="rounded-2xl p-5" style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
          <div className="flex items-center justify-between mb-1">
            ...
            <button onClick={() => setShowAddMcp(!showAddMcp)}
              ...
              style={{ background:'var(--blush)',color:'#fff' }}>+ 서버 추가</button>
          </div>
          ...
          {showAddMcp && (
            <div className="rounded-xl p-4 mb-4 flex flex-col gap-2" style={{ background:'var(--bg2)',border:'1px solid var(--border)' }}>
              ...
              <input value={newMcpName} onChange={e => setNewMcpName(e.target.value)} placeholder="서버 이름"
                className="px-3 py-2 rounded-xl text-[12px] outline-none"
                style={{ background:'var(--card)',border:'1px solid var(--border)',color:'var(--text)' }} />
              <input value={newMcpUrl} onChange={e => setNewMcpUrl(e.target.value)} placeholder="서버 URL"
                className="px-3 py-2 rounded-xl text-[12px] outline-none"
                ...
                <div key={tool.id} className="rounded-xl p-3 flex flex-col"
                  ...
                  <div className="flex items-start justify-between mb-1.5">
                    ...
                    <button onClick={() => setMcpEnabled(prev => ({ ...prev, [tool.id]: !prev[tool.id] }))}
                      ...
                      <div className="w-4 h-4 rounded-full absolute top-0.5 bg-white shadow-sm"
                        style={{ left:isOn?'18px':'2px',transition:'left .2s' }} />
                    </button>
                  </div>
                    ...
                    {isEdit ? (
                      ...
                      <div className="flex flex-wrap gap-1 items-center">
                        ...
                        <button onClick={() => setEditingMcp(tool.id)}
                          className="text-[9px] px-1 py-0.5 rounded ml-auto" style={{ color:'var(--muted)' }}>✏️</button>
                      </div>
                    )}
                  </div>
                </div>
