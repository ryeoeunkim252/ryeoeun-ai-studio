          ...
          {showAddTeam && (
            <div className="rounded-xl p-4 mt-3 flex flex-col gap-2" style={{ background: 'var(--bg2)', border: '1px solid var(--border)' }}>
              ...
              <div className="flex flex-wrap gap-1.5">
                {ICON_OPTIONS.map(ic => (
                  <button key={ic} onClick={() => setNewTeamIcon(ic)}
                    ...
                    style={{ background: newTeamIcon === ic ? 'var(--blush)' : 'var(--card)', border: '1px solid var(--border)' }}>
                    {ic}
                  </button>
                ))}
              </div>
              <input value={newTeamName} onChange={e => setNewTeamName(e.target.value)}
                placeholder="팀 이름 (예: 디자인 팀)"
              ...
              <input value={newTeamRole} onChange={e => setNewTeamRole(e.target.value)}
                placeholder="역할 (예: UI/UX 디자인)"
                className="px-3 py-2 rounded-xl text-[12px] outline-none"
                style={{ background: 'var(--card)', border: '1px solid var(--border)', color: 'var(--text)' }} />
              <input value={newTeamDesc} onChange={e => setNewTeamDesc(e.target.value)}
                placeholder="팀 설명 (예: 화면 설계와 사용자 경험을 담당해요)"
                className="px-3 py-2 rounded-xl text-[12px] outline-none"
                style={{ background: 'var(--card)', border: '1px solid var(--border)', color: 'var(--text)' }} />
              ...
              <div className="flex gap-2 mt-1">
                ...
                <button onClick={addTeam}
                  className="flex-1 py-2 rounded-xl text-[12px]"
                  style={{ background: 'var(--blush)', color: '#fff' }}>팀 추가</button>
              </div>
            </div>
          )}
              ...
              <div key={tool.id} className="rounded-xl p-3 flex flex-col"
                ...
                <div className="flex items-start justify-between mb-2">
                  ...
                  <button onClick={() => setMcpEnabled(prev => ({ ...prev, [tool.id]: !prev[tool.id] }))}
                    ...
                    <div className="w-4 h-4 rounded-full absolute top-0.5 bg-white shadow-sm"
                      style={{ left: mcpEnabled[tool.id] ? '18px' : '2px', transition: 'left .2s' }} />
                  </button>
                </div>
